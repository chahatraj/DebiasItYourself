...
_eval_shared_spec = importlib.util.spec_from_file_location("eval_shared_for_reasoning", _EVAL_SHARED_PATH)
if _eval_shared_spec is None or _eval_shared_spec.loader is None:
    raise ImportError(f"Could not import {_EVAL_SHARED_PATH}")
_eval_shared_mod = importlib.util.module_from_spec(_eval_shared_spec)
_eval_shared_spec.loader.exec_module(_eval_shared_mod)
build_scored_from_sentence_scores = _eval_shared_mod.build_scored_from_sentence_scores
compute_metrics_from_scored = _eval_shared_mod.compute_metrics_from_scored
flatten_stereoset_examples = _eval_shared_mod.flatten_stereoset_examples
load_crowspairs_df = _eval_shared_mod.load_crowspairs_df
load_stereoset_data = _eval_shared_mod.load_stereoset_data
make_crowspairs_eval_pairs = _eval_shared_mod.make_crowspairs_eval_pairs
compute_bbq_metrics_row = _eval_shared_mod.compute_bbq_metrics_row
sequence_logprob_batch_from_texts = _eval_shared_mod.sequence_logprob_batch_from_texts
stereoset_score = _eval_shared_mod.stereoset_score

_BBQ_EVAL_PATH = Path(__file__).resolve().parent / "8_bbq_eval_shared.py"
_bbq_eval_spec = importlib.util.spec_from_file_location("bbq_eval_shared_for_reasoning", _BBQ_EVAL_PATH)
if _bbq_eval_spec is None or _bbq_eval_spec.loader is None:
    raise ImportError(f"Could not import {_BBQ_EVAL_PATH}")
_bbq_eval_mod = importlib.util.module_from_spec(_bbq_eval_spec)
_bbq_eval_spec.loader.exec_module(_bbq_eval_mod)
evaluate_prediction_directory = _bbq_eval_mod.evaluate_prediction_directory
...
class TriggerTokenModel:
    """
    Wraps a causal LM to prepend strategy-specific trigger tokens before each
    input at inference time.

    The three step tokens (e.g. <SR_STEP1>, <SR_STEP2>, <SR_STEP3>) are added
    to the tokenizer vocabulary, the model's embedding table is resized, and
    each new embedding is initialised from the mean embedding of its
    corresponding step-description text.  At inference time the trigger token
    IDs are prepended to every input so the model "sees" the debiasing steps
    ...
    def __init__(
        ...
        step_tokens: List[str] = STEP_TOKENS[strategy]
        step_descs: List[str] = STRATEGY_STEPS[strategy]

        # 1. Register new special tokens and resize embeddings
        new_tokens = [t for t in step_tokens if t not in tokenizer.get_vocab()]
        if new_tokens:
            tokenizer.add_special_tokens({"additional_special_tokens": new_tokens})
            model.resize_token_embeddings(len(tokenizer))

        # 2. Initialise each trigger token embedding from its step description
        with torch.no_grad():
            emb = model.get_input_embeddings()
            for token, desc in zip(step_tokens, step_descs):
                tid = tokenizer.convert_tokens_to_ids(token)
                desc_ids = tokenizer(desc, add_special_tokens=False)["input_ids"]
                if desc_ids:
                    init_vec = emb.weight.data[desc_ids].float().mean(0)
                    emb.weight.data[tid] = init_vec.to(emb.weight.dtype)

        # 3. Cache trigger IDs as a (1, N_TRIGGERS) tensor
        self.trigger_ids: List[int] = [
            tokenizer.convert_tokens_to_ids(t) for t in step_tokens
        ]
        self._trigger_t = torch.tensor([self.trigger_ids], dtype=torch.long)

    # ------------------------------------------------------------------
    def _prepend_triggers(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Prepend trigger token IDs to input_ids and extend attention_mask."""
        bs = input_ids.size(0)
        trig = self._trigger_t.expand(bs, -1).to(input_ids.device)
        trig_mask = torch.ones(
            bs, self.N_TRIGGERS, dtype=attention_mask.dtype, device=attention_mask.device
        )
        return torch.cat([trig, input_ids], dim=1), torch.cat([trig_mask, attention_mask], dim=1)
    ...
    def sequence_logprob_batch(
        ...
        """
        Log-probability of each sentence under the model with trigger tokens
        prepended.  Only the original sentence tokens (not the trigger tokens)
        contribute to the returned score.
        """
        logprobs: List[float] = []
        n = self.N_TRIGGERS
        ...
        for start in range(0, len(sentences), batch_size):
            ...
            enc = self.tokenizer(
                ...
                padding=True,
                truncation=True,
                max_length=max_length - n,  # reserve room for trigger tokens
            )
            input_ids = enc["input_ids"].to(self.model.device)
            attn_mask = enc["attention_mask"].to(self.model.device)

            full_ids, full_mask = self._prepend_triggers(input_ids, attn_mask)
            outputs = self.model(input_ids=full_ids, attention_mask=full_mask)
            lp = torch.log_softmax(outputs.logits, dim=-1)  # [B, n+L, V]

            # shift: lp[:, i, :] predicts the token at full position i+1
            shift_lp = lp[:, :-1, :]       # [B, n+L-1, V]
            shift_ids = full_ids[:, 1:]     # [B, n+L-1]

            # Build a score mask: position (n-1+j) in the shifted sequence
            # predicts original token j (from input_ids), so score it iff
            # attn_mask[:, j] == 1.
            B, full_len = full_ids.shape
            L = input_ids.size(1)
            score_mask = torch.zeros(B, full_len - 1, device=input_ids.device, dtype=torch.float)
            score_mask[:, n - 1 : n - 1 + L] = attn_mask.float()

            safe_ids = shift_ids.clamp(min=0)
            token_lp = shift_lp.gather(-1, safe_ids.unsqueeze(-1)).squeeze(-1)
            token_lp = token_lp * score_mask
            logprobs.extend(token_lp.sum(dim=1).cpu().tolist())
    ...
    def completion_logprob(
        ...
        """
        For each completion string compute the log-probability of its tokens
        given (trigger_tokens + prompt_tokens) as context.
        """
        n = self.N_TRIGGERS
        scores: List[float] = []
        ...
        for comp in completions:
            ...
            # Positions in input_ids that correspond to completion tokens
            comp_positions = [
            ...
            shift_lp = lp[:, :-1, :]  # [1, n+L-1, V]

            score = 0.0
            for pos in comp_positions:
                if pos == 0:
                    continue
                # In full_ids this token is at position n+pos; the logit that
                # predicts it is at shift position n+pos-1.
                logit_pos = n + pos - 1
                tok = int(input_ids[0, pos].item())
                score += float(shift_lp[0, logit_pos, tok].item())
            scores.append(score)
...
def run_eval_crows(args: argparse.Namespace) -> None:
    ...
    if args.debias_method == "token_insertion":
        trigger_model = TriggerTokenModel(model, tokenizer, strategy)
        print("Scoring sent_more sentences with trigger tokens...")
        sent_more_scores = trigger_model.sequence_logprob_batch(
            sent_more, batch_size=args.batch_size, max_length=args.max_length
        )
        print("Scoring sent_less sentences with trigger tokens...")
        sent_less_scores = trigger_model.sequence_logprob_batch(
            sent_less, batch_size=args.batch_size, max_length=args.max_length
        )
    else:
        sent_more_scored = [
            apply_method_to_content(text, args.debias_method, strategy) for text in sent_more
        ]
        sent_less_scored = [
            apply_method_to_content(text, args.debias_method, strategy) for text in sent_less
        ]
...
def run_eval_stereoset(args: argparse.Namespace) -> None:
    ...
    if args.debias_method == "token_insertion":
        trigger_model = TriggerTokenModel(model, tokenizer, strategy)
        raw_sentences = [r["sentence"] for r in records]
        scores = trigger_model.sequence_logprob_batch(
            raw_sentences, batch_size=args.batch_size, max_length=args.max_length
        )
    else:
...
def run_infer_bbq(args: argparse.Namespace) -> None:
    ...
    # Pre-build TriggerTokenModel once so we only pay setup cost once
    trigger_model: Optional[TriggerTokenModel] = None
    if args.debias_method == "token_insertion":
        trigger_model = TriggerTokenModel(model, tokenizer, strategy)
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="BBQ inference"):
        ...
        try:
            if args.debias_method == "token_insertion":
                # Score each option with trigger tokens prepended to the prompt
                logprobs = trigger_model.completion_logprob(
                    base_prompt, options, max_length=args.max_length
                )
                ...
                if len(finite) == 0:
                    ...
                    m = float(np.max(arr[np.isfinite(arr)]))
                ...
                draft_text = ""
                final_text = ""
            elif args.debias_method == "post_inference_correction":
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_stereo.add_argument("--max_length", type=int, default=512)
    ...
    p_bbq_inf.add_argument("--batch_size", type=int, default=4)
    p_bbq_inf.add_argument("--max_length", type=int, default=1024)
