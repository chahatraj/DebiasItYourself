...
def mean_strategy_bbq_metric(config: PlotConfig, kind: str, shot: int, metric: str) -> tuple[float, str] | None:
    if config.strategy_key != "all_strategies":
        return None
    if shot != 1 or kind not in {"icl", "ft_icl"}:
        return None
    if kind == "ft_icl" and config.it_key != "allversions":
        return None
    ...
    for i, (dataset_key, panel_label, metric, metric_label, ideal) in enumerate(core.PANELS):
        for j, method in enumerate(METHODS):
            ...
            if kind in {"icl", "ft_icl"}:
                ...
                derived = mean_strategy_bbq_metric(config, kind, shot, metric) if dataset_key == "bbq" else None
