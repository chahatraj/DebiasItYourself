#!/usr/bin/env python3
"""Llama-70B versions of the paper figures.

The Llama-8B figures are built from an aggregate CSV. Llama-70B results currently
live as separate metric files, so this script reads the metric CSVs directly
and writes Llama-70B-only figures under figures/llama70b/.
"""
...
OUTDIR = ROOT / "figures" / "llama70b"
RESULTS = ROOT / "results"
NEW_RESULTS = RESULTS / "new_results"

BASE_ROOT = (
    NEW_RESULTS
    / "m6_llama70b_original5"
    / "m6_llama70b_original5_20260415_195536"
    / "plain_direct"
)
IT_ROOT = NEW_RESULTS / "m3_finetune_llama70b_ms500"
M6_ROOT = (
    NEW_RESULTS
    / "m6_llama70b_original5"
    / "m6_llama70b_original5_20260415_195536"
...
M6_REASONING_ROOT = (
    NEW_RESULTS
    / "m6_llama70b_reasoning"
    / "m6_llama70b_reasoning_20260506"
    / "m6"
)
ICL_ROOT = (
    NEW_RESULTS
...
BASELINES_ROOT = RESULTS / "11_baselines_llama70b"
BASELINE_RUN_ROOTS = [
    BASELINES_ROOT / "baseline_additions_20260513_20260513_133647",
    *sorted(BASELINES_ROOT.glob("completion_missing*"), reverse=True),
    BASELINES_ROOT / "baselines_llama70b_sep_20260414_001337",
    BASELINES_ROOT / "baselines_llama70b_retry_failed_20260414_072938",
    BASELINES_ROOT / "l70b_retry_lftf_peft_20260414_095445",
    BASELINES_ROOT / "llama70b_missing_baselines_20260415_075622",
    BASELINES_ROOT / "llama70b_baselines_retry_contrib_20260415_080908",
]
...
BASELINE_METHODS = [
    ...
    ("reduce_social_bias", "Reduce Social Bias"),
    ("selfdebias_reprompting", "SelfDebias: Reprompt"),
]
...
PARETO_LABEL_OFFSETS = {
    ...
    "DebiasLLMs": (8, -8),
    "DebiasNLG": (8, 4),
    "Reduce Social Bias": (-15, 31),
    "SelfDebias: Reprompt": (8, 12),
    "ICL": (8, 10),
    "DIY IT": (14, -6),
...
PARETO_SHORT_LABELS = {
    "Reduce Social Bias": "RSB",
    "SelfDebias: Reprompt": "SelfDebias\nReprompt",
    "ICL": "ICL",
    "DIY IT": "DIY IT",
...
def base_bias_path(dataset_key: str) -> Path | None:
    if dataset_key == "crowspairs":
        return BASE_ROOT / "evalshared/crows_pairs_metrics_overall_llama70b_plain_base_crowspairs.csv"
    if dataset_key == "stereoset":
        return BASE_ROOT / "evalshared/stereoset_metrics_llama70b_plain_base_stereoset.csv"
    if dataset_key == "bbq":
        return BASE_ROOT / "bbq/bbq_metrics_llama70b_plain_base_bbq.csv"
    if dataset_key == "winobias":
        return (
            BASE_ROOT
            / "evalshared/winobias/llama70b_plain_base_winobias/winobias_metrics_overall_llama70b_plain_base_winobias.csv"
        )
    if dataset_key == "winogender":
        return (
            BASE_ROOT
            / "evalshared/winogender/llama70b_plain_base_winogender/winogender_metrics_overall_llama70b_plain_base_winogender.csv"
        )
    return None
...
def it_bias_path(dataset_key: str, tag: str = "all_allversions") -> Path | None:
    if dataset_key == "bbq":
        return IT_ROOT / f"bbq/bbq_metrics_m3_llama70b_ms500_{tag}_bbq.csv"
    if dataset_key == "crowspairs":
        return IT_ROOT / f"evalshared/crows_pairs_metrics_overall_m3_llama70b_ms500_{tag}_crowspairs.csv"
    if dataset_key == "stereoset":
        return IT_ROOT / f"evalshared/stereoset_metrics_m3_llama70b_ms500_{tag}_stereoset.csv"
    if dataset_key == "winobias":
        return (
            IT_ROOT
            / f"evalshared/winobias/m3_llama70b_ms500_{tag}_winobias/winobias_metrics_overall_m3_llama70b_ms500_{tag}_winobias.csv"
        )
    if dataset_key == "winogender":
        return (
            IT_ROOT
            / f"evalshared/winogender/m3_llama70b_ms500_{tag}_winogender/winogender_metrics_overall_m3_llama70b_ms500_{tag}_winogender.csv"
        )
    return None
...
def m6_dir_path(shot: int, checkpoint: str, strategy: str) -> Path:
    ...
    return M6_ROOT / "eval" / f"m6_l70b_two_pass_{shot_word}__{checkpoint}__{strategy}"
...
def icl_bias_path(dataset_key: str, shot: int = 0) -> Path | None:
    ...
    if dataset_key == "bbq":
        return root / f"bbq/bbq_metrics_m4_baseicl_{shot_word}_llama70b_allstrat_bbq.csv"
    if dataset_key == "crowspairs":
        return root / f"evalshared/crows_pairs_metrics_overall_m4_baseicl_{shot_word}_llama70b_allstrat_crowspairs.csv"
    if dataset_key == "stereoset":
        return root / f"evalshared/stereoset_metrics_m4_baseicl_{shot_word}_llama70b_allstrat_stereoset.csv"
    if dataset_key == "winobias":
        return (
            root
            / f"evalshared/winobias/m4_baseicl_{shot_word}_llama70b_allstrat_winobias/"
            / f"winobias_metrics_overall_m4_baseicl_{shot_word}_llama70b_allstrat_winobias.csv"
        )
    if dataset_key == "winogender":
        return (
            root
            / f"evalshared/winogender/m4_baseicl_{shot_word}_llama70b_allstrat_winogender/"
            / f"winogender_metrics_overall_m4_baseicl_{shot_word}_llama70b_allstrat_winogender.csv"
        )
    return None
...
def ft_icl_bias_path(dataset_key: str, shot: int, tag: str) -> Path | None:
    ...
    shot_word = "one" if shot == 1 else "zero"
    if dataset_key == "bbq":
        return root / f"bbq/bbq_metrics_m4_fticl_{shot_word}_llama70b_{tag}_bbq.csv"
    if dataset_key == "crowspairs":
        return root / f"evalshared/crows_pairs_metrics_overall_m4_fticl_{shot_word}_llama70b_{tag}_crowspairs.csv"
    if dataset_key == "stereoset":
        return root / f"evalshared/stereoset_metrics_m4_fticl_{shot_word}_llama70b_{tag}_stereoset.csv"
    if dataset_key == "winobias":
        return (
            root
            / f"evalshared/winobias/m4_fticl_{shot_word}_llama70b_{tag}_winobias/"
            / f"winobias_metrics_overall_m4_fticl_{shot_word}_llama70b_{tag}_winobias.csv"
        )
    if dataset_key == "winogender":
        return (
            root
            / f"evalshared/winogender/m4_fticl_{shot_word}_llama70b_{tag}_winogender/"
            / f"winogender_metrics_overall_m4_fticl_{shot_word}_llama70b_{tag}_winogender.csv"
        )
    return None
...
def icl_strategy_bias_path(dataset_key: str, strategy_key: str) -> Path | None:
    ...
    if dataset_key == "bbq":
        return ICL_ROOT / f"bbq/bbq_metrics_m4_baseicl_zero_llama70b_{tag}_bbq.csv"
    if dataset_key == "crowspairs":
        return ICL_ROOT / f"evalshared/crows_pairs_metrics_overall_m4_baseicl_zero_llama70b_{tag}_crowspairs.csv"
    if dataset_key == "stereoset":
        return ICL_ROOT / f"evalshared/stereoset_metrics_m4_baseicl_zero_llama70b_{tag}_stereoset.csv"
    if dataset_key == "winobias":
        return (
            ICL_ROOT
            / f"evalshared/winobias/m4_baseicl_zero_llama70b_{tag}_winobias/"
            / f"winobias_metrics_overall_m4_baseicl_zero_llama70b_{tag}_winobias.csv"
        )
    if dataset_key == "winogender":
        return (
            ICL_ROOT
            / f"evalshared/winogender/m4_baseicl_zero_llama70b_{tag}_winogender/"
            / f"winogender_metrics_overall_m4_baseicl_zero_llama70b_{tag}_winogender.csv"
        )
    return None
...
def baseline_bias_path(dataset_key: str, method_key: str) -> Path | None:
    ...
    if dataset_key == "winobias":
        ...
        if method_key in {"bba", "cal"}:
            return first_glob(RESULTS / "3_baselines" / method_key / "winobias", ["**/winobias_metrics_overall*.csv"])
    if dataset_key == "winogender":
        direct = find_in_dirs(["winogender/**/winogender_metrics_overall*.csv"])
        if direct is not None:
            return direct
        if method_key in {"bba", "cal"}:
            return first_glob(RESULTS / "3_baselines" / method_key / "winogender", ["**/winogender_metrics_overall*.csv"])
    return None
...
def read_icl_bias_source(
    ...
    strategy_paths = sorted(ICL_ROOT.glob("bbq/bbq_metrics_m4_baseicl_zero_llama70b_*_bbq.csv"))
    values = []
    sources = []
...
def plot_debiasing_method_bars(records: list[dict[str, str]]) -> None:
    ...
    fig.suptitle("Llama-70B debiasing performance across benchmarks", y=0.992, fontsize=17, fontweight="bold", color="#111827")
    fig.text(
        0.01,
        0.043,
    ...
    for dataset_key, panel_label, metric, metric_label, ideal in PANELS:
        for method_key, method_label, group in method_order:
            if method_key == "base":
                ...
                path = m6_bias_path(dataset_key, 0, "base_all_strategies", "all_strategies")
            elif method_key == "diy_twopass_it":
                path = m6_bias_path(dataset_key, 0, "all_allversions", "all_strategies")
            else:
...
def plot_baseline_lollipop(records: list[dict[str, str]]) -> None:
    ...
    handles = [
        ...
        Line2D([0], [0], marker="o", color="none", markerfacecolor=styles["baseline"]["color"], markeredgecolor="#27313A", label="Existing baselines", markersize=7.5),
    ...
    fig.suptitle("Llama-70B DIY compared with existing debiasing baselines", y=0.99, fontsize=18, fontweight="bold", color="#111827")
    fig.text(
        0.01,
        0.035,
        "Metrics and normalization match the debiasing-performance figure; lower bias error is better. N/A marks unavailable Llama-70B metric files.",
        ha="left",
        va="bottom",
        fontsize=9.2,
        color="#374151",
...
def reasoning_path(method_key: str, dataset_key: str, shot: int = 0, tag: str = "all_allversions") -> Path | None:
    if method_key == "base":
        return first_glob(M6_REASONING_ROOT / "reasoning_first_pass" / dataset_key, [f"**/{dataset_key}_metrics_overall*.csv"]) or first_glob(M6_ROOT / "reasoning_first_pass" / dataset_key, [f"**/{dataset_key}_metrics_overall*.csv"])
    if method_key == "icl":
        return (
            ICL_REASONING_ROOT
            / "reasoning"
            / dataset_key
            / f"m4_baseicl_zero_llama70b_allstrat_{dataset_key}"
            / f"{dataset_key}_metrics_overall_m4_baseicl_zero_llama70b_allstrat_{dataset_key}.csv"
        )
    if method_key == "diy_it":
        return first_glob(
            IT_ROOT / "reasoning" / dataset_key,
            [f"m3_llama70b_ms500_{tag}_{dataset_key}/{dataset_key}_metrics_overall_m3_llama70b_ms500_{tag}_{dataset_key}.csv"],
        )
    if method_key == "diy_twopass_no_it":
        return m6_reasoning_path(dataset_key, shot, "base_all_strategies", "all_strategies")
    if method_key == "diy_twopass_it":
        return m6_reasoning_path(dataset_key, shot, "all_allversions", "all_strategies")
    return None


def m6_reasoning_dir_path(shot: int, checkpoint: str, strategy: str) -> Path:
    shot_word = "zero" if shot == 0 else "one" if shot == 1 else "two"
    return M6_REASONING_ROOT / "eval" / f"m6_l70b_reasoning_two_pass_{shot_word}__{checkpoint}__{strategy}"


def m6_reasoning_path(dataset_key: str, shot: int, checkpoint: str, strategy: str) -> Path | None:
    return first_glob(m6_reasoning_dir_path(shot, checkpoint, strategy) / dataset_key, [f"**/{dataset_key}_metrics_overall*.csv"])


def collect_reasoning_checkpoint_records() -> list[dict[str, str]]:
    records = []
    for dataset_key, benchmark_label in REASONING:
        for config in CONFIGS:
            path = reasoning_path("diy_it", dataset_key, tag=config.it_tag)
            if path is None or not path.exists():
                record_missing("reasoning_performance_by_checkpoint", config.title, dataset_key, benchmark_label, "reasoning metric file not available")
                continue
            try:
                acc = read_accuracy(path)
            except (KeyError, RuntimeError, ValueError) as exc:
                record_missing("reasoning_performance_by_checkpoint", config.title, dataset_key, benchmark_label, str(exc))
                continue
            records.append(
                {
                    "dataset_key": dataset_key,
                    "benchmark_label": benchmark_label,
                    "checkpoint_key": config.it_tag,
                    "checkpoint_label": config.title,
                    "strategy_label": config.strategy_label,
                    "ft_label": config.ft_label,
                    "metric": "accuracy",
                    "accuracy": f"{acc:.8g}",
...
def collect_reasoning_intervention_records() -> list[dict[str, str]]:
    records = []
    specs = [
        ("base", "Base Model", "base", "base", None),
        ("all_strategies", "All interventions", "all_strategies", "base_all_strategies", "all_strategies"),
    ] + [
        (tag, intervention_label, intervention_key, f"base_{intervention_key}", intervention_key)
        for tag, intervention_key, _, intervention_label in INTERVENTIONS
    ]
    for dataset_key, benchmark_label in REASONING:
        for method_key, method_label, strategy_key, checkpoint, strategy in specs:
            if strategy is None:
                path = reasoning_path("base", dataset_key)
            else:
                path = m6_reasoning_path(dataset_key, 0, checkpoint, strategy)
            if path is None or not path.exists():
                record_missing("reasoning_performance_by_intervention", method_label, dataset_key, benchmark_label, "reasoning metric file not available")
                continue
            try:
                acc = read_accuracy(path)
            except (KeyError, RuntimeError, ValueError) as exc:
                record_missing("reasoning_performance_by_intervention", method_label, dataset_key, benchmark_label, str(exc))
                continue
            records.append(
                {
                    "dataset_key": dataset_key,
...
def baseline_reasoning_path(method_key: str, dataset_key: str) -> Path | None:
    ...
    for method_dir in method_dirs:
        for path in sorted((method_dir / dataset_key).glob(f"**/{dataset_key}_metrics_overall*.csv")):
            if method_key.startswith("selfdebias") and not selfdebias_path_matches(path):
                continue
            return path
    return None
...
def plot_reasoning(records: list[dict[str, str]]) -> None:
    ...
    if not finite:
        ax.text(
            0.5,
            0.55,
            "No comparable Llama-70B reasoning benchmark files found.",
            ha="center",
            va="center",
    ...
    ax.set_title("Llama-70B reasoning benchmark performance", pad=10, fontsize=16.5, fontweight="bold", color="#111827")
    ...
    fig.text(
        0.01,
        0.026,
        "Metric: raw benchmark accuracy; higher is better. Empty panels indicate that comparable Llama-70B reasoning metrics were not found in the current result folders.",
        ha="left",
        va="bottom",
        fontsize=9.2,
        color="#374151",
...
def plot_reasoning_checkpoints(records: list[dict[str, str]]) -> None:
    set_style("small")
    checkpoint_keys = [cfg.it_tag for cfg in CONFIGS]
    label_map = {
        "all_allversions": "All\nversions",
        "all_opinion": "All\nopinion",
        "all_action": "All\naction",
    ...
    hatches = ["///", "\\\\\\", "...", "xxx", "---", "++", "oo", "**", ""]
    values = {
        (row["dataset_key"], row["checkpoint_key"]): float(row["accuracy_percent_plotted"])
        for row in records
    }
    fig, axes = plt.subplots(1, 3, figsize=(14.6, 4.8), sharey=False)
    fig.patch.set_facecolor("white")
    x = np.arange(len(checkpoint_keys))
    for ax, (dataset_key, benchmark_label) in zip(axes, REASONING):
        ys = np.array([values.get((dataset_key, key), np.nan) for key in checkpoint_keys], dtype=float)
        ax.set_facecolor("#FCFCFD")
        bars = ax.bar(x, ys, width=0.72, color=colors, edgecolor="#20252A", linewidth=0.95, zorder=3)
        for bar, hatch in zip(bars, hatches):
            bar.set_hatch(hatch)
        finite = ys[np.isfinite(ys)]
        if len(finite):
            ymin = max(0, float(np.min(finite)) - 5.5)
            ymax = min(100, float(np.max(finite)) + 6.5)
            if ymax - ymin < 12:
                mid = (ymin + ymax) / 2
                ymin = max(0, mid - 6)
                ymax = min(100, mid + 6)
            ...
            ax.set_ylim(0, 100)
            ax.text(0.5, 0.54, "N/A", ha="center", va="center", transform=ax.transAxes, fontsize=12, fontweight="bold", color="#6B7280")
        ymin, ymax = ax.get_ylim()
        offset = 0.025 * (ymax - ymin)
        for bar, value in zip(bars, ys):
            if np.isfinite(value):
                ax.text(bar.get_x() + bar.get_width() / 2, value + offset, f"{value:.1f}", ha="center", va="bottom", fontsize=8.2, fontweight="bold", color="#111827")
        annotate_missing_bars(ax, x, ys)
        ax.set_title(benchmark_label, pad=9, fontsize=12.5, fontweight="bold", color="#111827")
        ax.set_xticks(x)
        ax.set_xticklabels([label_map.get(key, key.replace("_", "\n")) for key in checkpoint_keys], rotation=0)
        ax.grid(axis="y", color="#DCE2EA", linewidth=0.78, linestyle="--", alpha=0.7)
        ax.set_axisbelow(True)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ax.spines["left"].set_color("#AEB7C2")
        ax.spines["bottom"].set_color("#AEB7C2")
        ax.tick_params(axis="x", length=0, pad=6, colors="#374151")
        ax.tick_params(axis="y", colors="#374151")
    axes[0].set_ylabel("Accuracy (%)", fontsize=10.2, fontweight="semibold", color="#374151")
    fig.suptitle("Llama-70B reasoning performance by DIY IT checkpoint", y=0.985, fontsize=16.2, fontweight="bold", color="#111827")
    fig.text(
        0.01,
        0.028,
        "Metric: raw benchmark accuracy; higher is better. Each bar is one DIY instruction-tuned checkpoint. N/A marks unavailable checkpoint/benchmark cells.",
        ha="left",
        va="bottom",
        fontsize=8.8,
        color="#374151",
    )
    fig.tight_layout(rect=(0.0, 0.08, 1, 0.91), w_pad=1.2)
    fig.savefig(OUTDIR / "pdf/reasoning_performance_by_checkpoint.pdf", bbox_inches="tight")
    plt.close(fig)


def plot_reasoning_interventions(records: list[dict[str, str]]) -> None:
    set_style("small")
    method_keys = ["base", "all_strategies"] + [tag for tag, _, _, _ in INTERVENTIONS]
    label_map = {
        "base": "Base",
        "all_strategies": "All\ninterv.",
        "sr": "Stereo.\nreplace",
    ...
    values = {
        (row["dataset_key"], row["method_key"]): float(row["accuracy_percent_plotted"])
        for row in records
    }
    fig, axes = plt.subplots(1, 3, figsize=(12.8, 4.65), sharey=False)
    fig.patch.set_facecolor("white")
    x = np.arange(len(method_keys))
    for ax, (dataset_key, benchmark_label) in zip(axes, REASONING):
        ys = np.array([values.get((dataset_key, key), np.nan) for key in method_keys], dtype=float)
        ax.set_facecolor("#FCFCFD")
        bars = ax.bar(
            x,
            ys,
            width=0.72,
            color=[colors[key] for key in method_keys],
            edgecolor="#20252A",
            linewidth=0.95,
            zorder=3,
        )
        ...
        ax.set_title(benchmark_label, pad=9, fontsize=12.5, fontweight="bold", color="#111827")
        ax.set_xticks(x)
        ax.set_xticklabels([label_map[key] for key in method_keys])
        ax.grid(axis="y", color="#DCE2EA", linewidth=0.78, linestyle="--", alpha=0.7)
        ax.set_axisbelow(True)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ...
        ax.tick_params(axis="y", colors="#374151")
    axes[0].set_ylabel("Accuracy (%)", fontsize=10.2, fontweight="semibold", color="#374151")
    fig.suptitle("Llama-70B reasoning performance by two-pass intervention", y=0.985, fontsize=16.2, fontweight="bold", color="#111827")
    fig.text(
        0.01,
        0.028,
        "Metric: raw benchmark accuracy; higher is better. Bars use completed Llama-70B reasoning cells from the dedicated reasoning run; N/A marks unavailable cells.",
        ha="left",
        va="bottom",
        fontsize=8.8,
        color="#374151",
    )
    fig.tight_layout(rect=(0.0, 0.09, 1, 0.91), w_pad=1.05)
    fig.savefig(OUTDIR / "pdf/reasoning_performance_by_intervention.pdf", bbox_inches="tight")
    plt.close(fig)
...
def collect_pareto_records(
    ...
    # bias_records ensures core DIY methods are available even when a baseline
    # table is missing some entries.
...
def plot_pareto(records: list[dict[str, str]]) -> None:
    ...
    if points:
        ...
        ax.text(0.5, 0.55, "No Llama-70B methods have both bias and reasoning metrics.", ha="center", va="center", transform=ax.transAxes, fontsize=10.5, color="#374151")
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
    ...
    fig.text(
        ...
        "Lower bias and higher reasoning accuracy are better. Existing-baseline reasoning uses Llama-70B baseline utility runs when available.",
        ha="left",
        va="bottom",
        fontsize=6.8,
        color="#374151",
...
def collect_intervention_records() -> list[dict[str, str]]:
    ...
    for dataset_key, panel_label, metric, metric_label, ideal in PANELS:
        ...
        for tag, intervention_key, _, intervention_label in INTERVENTIONS:
            ...
            for method, method_key, path in (
                (
                    "DIY Two Pass (No IT)",
                    f"diy_twopass_no_it_{intervention_key}",
                    m6_strategy_path(dataset_key, 0, tag, intervention_key, with_it=False),
                ),
                (
                    "DIY Two Pass (IT)",
                    f"diy_twopass_it_{intervention_key}",
                    m6_strategy_path(dataset_key, 0, tag, intervention_key, with_it=True),
                ),
            ):
                two_pass = read_bias_source(
                    dataset_key,
                    panel_label,
                    metric,
                    metric_label,
...
def plot_intervention(records: list[dict[str, str]]) -> None:
    ...
    fig.text(0.5, 0.985, "Llama-70B bias error reduction by intervention strategy", ha="center", va="top", fontsize=16.5, fontweight="bold", color="#111827")
    fig.text(
        0.012,
        0.028,
        "Bars show absolute reduction from the base model on each normalized bias metric; higher is better. Missing Llama-70B strategy files are left blank.",
        ha="left",
        va="bottom",
        fontsize=8.8,
        color="#374151",
...
def collect_shot_records(tag: str, figure: str, title: str = "") -> list[dict[str, str]]:
    ...
    for dataset_key, panel_label, metric, metric_label, ideal in PANELS:
        for method in SHOT_METHODS:
            key = str(method["key"])
            if key == "base":
                label, path = "Base Model Inference", base_bias_path(dataset_key)
            elif key == "icl":
                if strategy_key == "all_strategies":
                    row = read_icl_bias_source(dataset_key, panel_label, metric, metric_label, ideal, figure)
                else:
                ...
                label, path = "ICL 1", icl_bias_path(dataset_key, shot=1)
            elif key.startswith("ft_icl_"):
                shot = int(key.rsplit("_", 1)[1])
                label = str(method["label"]).replace("\n", " ")
                path = ft_icl_bias_path(dataset_key, shot, tag)
            elif key == "it":
                label, path = "DIY IT", it_bias_path(dataset_key, tag)
            elif key.startswith("no_it_"):
                shot = int(key.rsplit("_", 1)[1])
                label = str(method["label"]).replace("\n", " ")
                path = m6_strategy_path(dataset_key, shot, tag, strategy_key, with_it=False)
            else:
                shot = int(key.rsplit("_", 1)[1])
                label = str(method["label"]).replace("\n", " ")
                path = m6_strategy_path(dataset_key, shot, tag, strategy_key, with_it=True)
...
def plot_shot_matrix(records: list[dict[str, str]], filename: str, title: str, subtitle: str) -> None:
    ...
    fig.text(
        0.01,
        0.028,
        "Metrics and normalization match the main debiasing plot. Shot labels 0/1/2 refer to two-pass demonstrations; N/A marks unavailable Llama-70B bias files.",
        ha="left",
        va="bottom",
        fontsize=9.3,
        color="#374151",
...
def main() -> None:
    ...
    if reasoning_records:
        ...
        write_empty_csv(
            OUTDIR / "csv/reasoning_performance_data.csv",
            [
                "dataset_key",
                "benchmark_label",
                "method_key",
                ...
                "metric",
                "accuracy",
                "accuracy_percent_plotted",
                "source_file",
            ],
        )
    plot_reasoning(reasoning_records)

    reasoning_checkpoint_records = collect_reasoning_checkpoint_records()
    if reasoning_checkpoint_records:
        write_csv(OUTDIR / "csv/reasoning_performance_by_checkpoint_data.csv", reasoning_checkpoint_records)
    else:
        write_empty_csv(
            OUTDIR / "csv/reasoning_performance_by_checkpoint_data.csv",
            [
                "dataset_key",
                "benchmark_label",
                "checkpoint_key",
    ...
    plot_reasoning_checkpoints(reasoning_checkpoint_records)

    reasoning_intervention_records = collect_reasoning_intervention_records()
    if reasoning_intervention_records:
        write_csv(OUTDIR / "csv/reasoning_performance_by_intervention_data.csv", reasoning_intervention_records)
    else:
        write_empty_csv(
            OUTDIR / "csv/reasoning_performance_by_intervention_data.csv",
            [
                "dataset_key",
                "benchmark_label",
                "method_key",
    ...
    plot_reasoning_interventions(reasoning_intervention_records)
    ...
    plot_shot_matrix(
        ...
        "Llama-70B debiasing performance by two-pass shot setting",
